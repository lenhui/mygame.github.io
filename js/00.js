...
// --------------------------------------------------
	...
	function shuffle(a) { //a is array
    	...
    	setTimeout(function(){
    		for(var i=0;i<54;i++){
    			$('.all_poker li').delay(35).eq(i).animate({left:0+'px',top:300+i+'px'},400).delay(500).animate({left:(Math.ceil(Math.random()*2000))-1000+'px',top:(Math.ceil(Math.random()*2000))-1000+'px'},400).delay(2000).animate({left:0+'px',top:0+i/2+'px'},400);
    		};
    	},800);
	    //陈总动画效果
		// console.log(a);
...
// --------------------------------------------------之前的应该先不用管了
	...
	function dealPoker() {
			...
			$('.player_avatar').css('display','block');
					...
					function play_skip(htmlclass) { //使得出牌跳牌按钮在地主位置出现
						...
						function playBtnFunc() {
							...
							function afterPlayDisplay(playerObj) { //生成牌的背景
								...
								console.log(playerObj.poker.length);
							...
							function playChange(playerObj) { //出牌后的视觉以及数据内容变化
								...
								var playCard_indexs = []; //数组元素指代打出的牌在手牌中的位置，通过它获取牌的点数
								var playCard_point = null;
								function kind_1Or5plus(argument) { //此函数用于判断是否打单张1或顺子105
									//是否符合出牌格式的出牌数量要求
									if (play_card_MessageObj.card_Arr.length!==1&&play_card_MessageObj.card_Arr.length<5) { game_manager.Isturn = false; alert('张数不符合顺子出牌规则'); return false; };
									//先判断桌面对应张数
									if ($('.game_table li.chosen_card').length!==0 && play_card_MessageObj.card_Arr.length!==$('.game_table li.chosen_card').length && game_manager.skipTime!==2) { alert('与上一次出牌的张数不对应相等'); game_manager.Isturn = false; return false; };
									for (var i = 0; i < play_card_MessageObj.card_Arr.length; i++) {
										...
										//------------------------------------------------------------------------
										//顺子大小判断
										else if(playCard_indexs.length===play_card_MessageObj.card_Arr.length && parseInt( game_manager.nowPlayer.poker[playCard_indexs[play_card_MessageObj.card_Arr.length-1]].split('-')[1] ) <= play_card_MessageObj.card_point && play_card_MessageObj.card_Type!==null){
										...
										if (play_card_MessageObj.card_Arr.length>=5) {play_card_MessageObj.card_Type = 105} ;
										//样式控制：牌的视觉变化
								...
								function kind_1(argument) { //此函数用于判断是否打单张1
									//是否符合出牌格式的出牌数量要求
									// if (play_card_MessageObj.card_Arr.length!==1) { game_manager.Isturn = false; alert('张数不符合单张出牌规则'); return false; };
									//先判断桌面对应张数
									if ($('.game_table li.chosen_card').length!==0 && play_card_MessageObj.card_Arr.length!==$('.game_table li.chosen_card').length && game_manager.skipTime!==2) { alert('与上一次出牌的张数不对应相等'); game_manager.Isturn = false; return false; };
									for (var i = 0; i < play_card_MessageObj.card_Arr.length; i++) {
										//拿到待出牌在手牌中的的index，才能拿到点数
										...
										//拿到牌的点数，然后判断牌点够不够大，不够大可以在这里退出
										if (game_manager.skipTime===2) { play_card_MessageObj.card_point=null; };
										if (play_card_MessageObj.card_point===null || playCard_point>play_card_MessageObj.card_point) {
											play_card_MessageObj.card_point = playCard_point;
										}
										//------------------------------------------------------------------------
								...
								// -------------------------------------------------------------------------------
								function kind_2(argument) { //此函数用于判断是否打对子2
									//是否符合出牌格式的出牌数量要求
									if (game_manager.skipTime===2) { play_card_MessageObj.card_Type=null; }
									console.log(play_card_MessageObj.card_Type);
									if (play_card_MessageObj.card_Type !== null && play_card_MessageObj.card_Type !== 2) {
										game_manager.Isturn = false; alert('这时不能出对子'); return false;
									}
									if (play_card_MessageObj.card_Arr.length!==2) { game_manager.Isturn = false; alert('张数不符合对子出牌规则'); return false; };
									//先判断桌面对应张数
									if ($('.game_table li.chosen_card').length!==0 && play_card_MessageObj.card_Arr.length!==$('.game_table li.chosen_card').length && game_manager.skipTime!==2) { alert('与上一次出牌的张数不对应相等'); game_manager.Isturn = false; return false; };

									var playCard_point_arr = [];
									...
									for (var i = 0; i < play_card_MessageObj.card_Arr.length; i++) {
										...
										//拿到牌的点数
										playCard_point_arr.push(parseInt(game_manager.nowPlayer.poker[playCard_indexs[i]].split('-')[1]));

									}
										// -------------------------------------------------------------这里创造一个for循环分解----------

									//拿到牌的点数，然后判断牌点够不够大，不够大可以在这里退出
									if (playCard_point_arr[0]!==playCard_point_arr[1]) {
										alert('这不是对子');
										game_manager.Isturn = false; 
										return false; 
									}
									playCard_point = playCard_point_arr[1];
									// if (play_card_MessageObj.card_point===null || playCard_point>play_card_MessageObj.card_point) {
									// 	play_card_MessageObj.card_point = playCard_point;
									// }
									//对子大小判断
									// console.log(play_card_MessageObj.card_point);
									// console.log(parseInt( game_manager.nowPlayer.poker[playCard_indexs[play_card_MessageObj.card_Arr.length-1]].split('-')[1] ));
									if(playCard_point <= play_card_MessageObj.card_point && play_card_MessageObj.card_Type!==null){ 
										alert('牌不够大');
										game_manager.Isturn = false; 
										return false; }
									else{
									...
									//根据一些线索和条件指定此时的出牌类型
									if (play_card_MessageObj.card_Arr.length===2) {play_card_MessageObj.card_Type = 2;} ;
									for (var i = 0; i < playCard_point_arr.length; i++) {
										//样式控制：牌的视觉变化
										if (i===0) { $('.game_table li.chosen_card').remove(); };
										play_card_MessageObj.card_Arr[i].clone().css({'top':'180px','left':(i*20-(play_card_MessageObj.card_Arr.length-1)/2*20)+'px'}).appendTo($('.game_table'));
									}//出牌数组循环结束
								...
								function kind_1415(argument) { //此函数用于判断是否打王炸1415
									//是否符合出牌格式的出牌数量要求
									if (play_card_MessageObj.card_Arr.length!==2) { game_manager.Isturn = false; alert('张数不符合王炸出牌规则'); return false; };
									...
									playCard_point_arr.push(14,15);
										// -------------------------------------------------------------这里创造一个for循环分解----------

									//拿到牌的点数，然后判断牌点够不够大，不够大可以在这里退出
									if (playCard_point_arr[0]!==14) { game_manager.Isturn = false; alert('不符合王炸出牌规则'); return false; }
									playCard_point = 15;
									// if (play_card_MessageObj.card_point===null || playCard_point>play_card_MessageObj.card_point) {
									// 	play_card_MessageObj.card_point = playCard_point;
									// }
									...
									//根据一些线索和条件指定此时的出牌类型
									play_card_MessageObj.card_Type = 1415;
									for (var i = 0; i < playCard_point_arr.length; i++) {
										//样式控制：牌的视觉变化
										if (i===0) { $('.game_table li.chosen_card').remove(); };
										play_card_MessageObj.card_Arr[i].clone().css({'top':'180px','left':(i*20-(play_card_MessageObj.card_Arr.length-1)/2*20)+'px'}).appendTo($('.game_table'));
								...
								function kind_30(argument) { //此函数用于判断是否打3张同点牌
									//是否符合出牌格式的出牌数量要求
									if (play_card_MessageObj.card_Type !== null && play_card_MessageObj.card_Type !== 30) {
										game_manager.Isturn = false; alert('这时不能出三个同点数的牌'); return false;
									}
									// if (play_card_MessageObj.card_Arr.length!==3) { game_manager.Isturn = false; alert('张数不符合对子出牌规则'); return false; };
									//先判断桌面对应张数
									if ($('.game_table li.chosen_card').length!==0 && play_card_MessageObj.card_Arr.length!==$('.game_table li.chosen_card').length && game_manager.skipTime!==2) { alert('与上一次出牌的张数不对应相等'); game_manager.Isturn = false; return false; };
									...
									//拿到牌的点数，然后判断牌点够不够大，不够大可以在这里退出
									if (   !( playCard_point_arr[0]===playCard_point_arr[1] && playCard_point_arr[0]===playCard_point_arr[2] )  ) {
										alert('这不是三顺');
										game_manager.Isturn = false; 
										return false; 
									}
									playCard_point = playCard_point_arr[0];
									// if (play_card_MessageObj.card_point===null || playCard_point>play_card_MessageObj.card_point) {
									// 	play_card_MessageObj.card_point = playCard_point;
									// }
									//对子大小判断
									...
									//根据一些线索和条件指定此时的出牌类型
									if (play_card_MessageObj.card_Arr.length===3) {play_card_MessageObj.card_Type = 30;} ;
									for (var i = 0; i < playCard_point_arr.length; i++) {
										//样式控制：牌的视觉变化
										if (i===0) { $('.game_table li.chosen_card').remove(); };
										play_card_MessageObj.card_Arr[i].clone().css({'top':'180px','left':(i*20-(play_card_MessageObj.card_Arr.length-1)/2*20)+'px'}).appendTo($('.game_table'));
								...
								function kind_4444(argument) { //此函数用于判断是否打普通炸弹4444
									//是否符合出牌格式的出牌数量要求
									if (play_card_MessageObj.card_Type === 1415 && play_card_MessageObj.card_point === 15) {
										game_manager.Isturn = false; alert('你遇到王炸了，炸弹都没用。'); return false;
									}
									// if (play_card_MessageObj.card_Arr.length!==3) { game_manager.Isturn = false; alert('张数不符合对子出牌规则'); return false; };
									//先判断桌面对应张数
									// if ($('.game_table li.chosen_card').length!==0 && play_card_MessageObj.card_Arr.length!==$('.game_table li.chosen_card').length && game_manager.skipTime!==2) { alert('与上一次出牌的张数不对应相等'); game_manager.Isturn = false; return false; };
									...
									//拿到牌的点数，然后判断牌点够不够大，不够大可以在这里退出
									// !( playCard_point_arr[0]===playCard_point_arr[1] && playCard_point_arr[0]===playCard_point_arr[2] && playCard_point_arr[0]===playCard_point_arr[3] ) 
									console.log(playCard_point_arr);
									if (  playCard_point_arr[0]!==playCard_point_arr[3]  ) {
										alert('普通炸弹必须四张的点数都相同');
										game_manager.Isturn = false; 
										return false; 
									}
									playCard_point = playCard_point_arr[0];
									...
									// console.log(play_card_MessageObj.card_point);
									// console.log(parseInt( game_manager.nowPlayer.poker[playCard_indexs[play_card_MessageObj.card_Arr.length-1]].split('-')[1] ));
									if(playCard_point <= play_card_MessageObj.card_point && play_card_MessageObj.card_Type===4444){ 
										alert('牌不够大');
										game_manager.Isturn = false; 
										return false; }
									else{
									...
									//根据一些线索和条件指定此时的出牌类型
									if (play_card_MessageObj.card_Arr.length===4) {play_card_MessageObj.card_Type = 4444;} ;
									for (var i = 0; i < playCard_point_arr.length; i++) {
										//样式控制：牌的视觉变化
										if (i===0) { $('.game_table li.chosen_card').remove(); };
										play_card_MessageObj.card_Arr[i].clone().css({'top':'180px','left':(i*20-(play_card_MessageObj.card_Arr.length-1)/2*20)+'px'}).appendTo($('.game_table'));
								...
								function kind_31(argument) { //此函数用于判断是否打三带一
									//是否符合出牌格式的出牌数量要求
									if (play_card_MessageObj.card_Type !== null && play_card_MessageObj.card_Type !== 31) {
										game_manager.Isturn = false; alert('这时不能出三带一'); return false;
									}
									// if (play_card_MessageObj.card_Arr.length!==4) { game_manager.Isturn = false; alert('张数不符合对子出牌规则'); return false; };
									//先判断桌面对应张数
									if ($('.game_table li.chosen_card').length!==0 && play_card_MessageObj.card_Arr.length!==$('.game_table li.chosen_card').length && game_manager.skipTime!==2) { alert('与上一次出牌的张数不对应相等'); game_manager.Isturn = false; return false; };
									...
									//拿到牌的点数，然后判断牌点够不够大，不够大可以在这里退出
									if (!(playCard_point_arr[1]===playCard_point_arr[2] && (playCard_point_arr[0]===playCard_point_arr[1]||playCard_point_arr[1]===playCard_point_arr[3]) && playCard_point_arr[0]!==playCard_point_arr[3])) {
										alert('这不是三带一');
										game_manager.Isturn = false; 
										return false; 
									}
									...
									//根据一些线索和条件指定此时的出牌类型
									if (play_card_MessageObj.card_Arr.length===4) {play_card_MessageObj.card_Type = 31;} ;
									for (var i = 0; i < playCard_point_arr.length; i++) {
										//样式控制：牌的视觉变化
										if (i===0) { $('.game_table li.chosen_card').remove(); };
										play_card_MessageObj.card_Arr[i].clone().css({'top':'180px','left':(i*20-(play_card_MessageObj.card_Arr.length-1)/2*20)+'px'}).appendTo($('.game_table'));
								...
								function kind_32(argument) { //此函数用于判断是否打三带二
									//是否符合出牌格式的出牌数量要求
									console.log('第二次有进来吗？');
									if (play_card_MessageObj.card_Type !== null && play_card_MessageObj.card_Type !== 32) {
										game_manager.Isturn = false; alert('这时不能出三带二'); return false;
									}
									if (play_card_MessageObj.card_Arr.length!==5) { game_manager.Isturn = false; alert('张数不符合三带二出牌规则'); return false; };
									//先判断桌面对应张数
									if ($('.game_table li.chosen_card').length!==0 && play_card_MessageObj.card_Arr.length!==$('.game_table li.chosen_card').length && game_manager.skipTime!==2) { alert('与上一次出牌的张数不对应相等'); game_manager.Isturn = false; return false; };
									...
									//拿到牌的点数，然后判断牌点够不够大，不够大可以在这里退出
									// console.log(typeof (playCard_point_arr[0]===playCard_point_arr[1]===playCard_point_arr[2] || playCard_point_arr[2]===playCard_point_arr[3]===playCard_point_arr[4]));
									// (playCard_point_arr[1]===playCard_point_arr[2] || playCard_point_arr[2]===playCard_point_arr[3])&&(playCard_point_arr[0]===playCard_point_arr[2] || playCard_point_arr[2]===playCard_point_arr[4])
									// console.log(game_manager.nowPlayer.poker);
									// console.log(playCard_point_arr);
									
									// console.log(playCard_point_arr[1]);
									// console.log(playCard_point_arr[2]);
									// console.log(playCard_point_arr[3]);
									if (  !(playCard_point_arr[1]===playCard_point_arr[2] || playCard_point_arr[2]===playCard_point_arr[3])  ) {
										alert('三带二里面没有三同点');
										game_manager.Isturn = false; 
										return false; 
									}
									// console.log(playCard_point_arr[0]);
									// console.log(playCard_point_arr[1]);
									// console.log(playCard_point_arr[3]);
									// console.log(playCard_point_arr[4]);
									if (playCard_point_arr[0]!==playCard_point_arr[1]) {
										alert('三带二里面没有二同点');
										game_manager.Isturn = false; 
										return false; 
									}
									if (playCard_point_arr[3]!==playCard_point_arr[4]) {
										alert('三带二里面没有二同点');
										game_manager.Isturn = false; 
										return false; 
									}
									playCard_point = playCard_point_arr[2];
									// if (play_card_MessageObj.card_point===null || playCard_point>play_card_MessageObj.card_point) {
									// 	play_card_MessageObj.card_point = playCard_point;
									// }
									//对子大小判断
									...
									//根据一些线索和条件指定此时的出牌类型
									if (play_card_MessageObj.card_Arr.length===5) {play_card_MessageObj.card_Type = 32;} ;
									for (var i = 0; i < playCard_point_arr.length; i++) {
										//样式控制：牌的视觉变化
										if (i===0) { $('.game_table li.chosen_card').remove(); };
										play_card_MessageObj.card_Arr[i].clone().css({'top':'180px','left':(i*20-(play_card_MessageObj.card_Arr.length-1)/2*20)+'px'}).appendTo($('.game_table'));
									...
									for (var j = 0; j < play_card_MessageObj.card_Arr.length; j++) {
										...
										console.log(playCard_indexs); 
										
										console.log(playCard_indexs[j]); 
										console.log(play_card_MessageObj.card_Arr[j].order); 
										console.log(playCard_indexs[j]-play_card_MessageObj.card_Arr[j].order); 
										// console.log(game_manager.nowPlayer.poker.splice(playCard_indexs[j]-play_card_MessageObj.card_Arr[j].order,1));
									}
								}

								function kind_105(argument) { //此函数用于判断是否打5张顺子
									//是否符合出牌格式的出牌数量要求
									if (play_card_MessageObj.card_Type !== null && play_card_MessageObj.card_Type !== 105) {
										game_manager.Isturn = false; alert('这时不能出顺子'); return false;
									}
									if (play_card_MessageObj.card_Arr.length<5) { game_manager.Isturn = false; alert('张数不符合对子出牌规则'); return false; };
									//先判断桌面对应张数
									if ($('.game_table li.chosen_card').length!==0 && play_card_MessageObj.card_Arr.length!==$('.game_table li.chosen_card').length && game_manager.skipTime!==2) { alert('与上一次出牌的张数不对应相等'); game_manager.Isturn = false; return false; };
									...
									//拿到牌的点数，然后判断牌点够不够大，不够大可以在这里退出
									// console.log(playCard_point_arr[Math.ceil(playCard_point_arr.length/2-1)]);
									// console.log( (playCard_point_arr[playCard_point_arr.length-1]+playCard_point_arr[0])/2 );
									if (   !(playCard_point_arr[Math.ceil(playCard_point_arr.length/2)-1]===(playCard_point_arr[playCard_point_arr.length-1]+playCard_point_arr[0])/2)     ) {
										alert('这不是5张顺子');
										game_manager.Isturn = false; 
										return false; 
									}
									playCard_point = playCard_point_arr[4];
									// if (play_card_MessageObj.card_point===null || playCard_point>play_card_MessageObj.card_point) {
									// 	play_card_MessageObj.card_point = playCard_point;
									// }
									//对子大小判断
								...
								// --------------------------------------------------------------------------------
								if (play_card_MessageObj.card_Arr.length===1) {
									kind_1();
								}
								else if (play_card_MessageObj.card_Arr.length===2) {
									kind_1415();
									// kind_2();
									if(play_card_MessageObj.card_Type!==1415){ kind_2(); }
								}
								else if (play_card_MessageObj.card_Arr.length===3) {
									kind_30();
								}
								else if (play_card_MessageObj.card_Arr.length===4) {
									kind_4444();
									// kind_31();
									if(play_card_MessageObj.card_Type!==4444){ kind_31(); }
								}
								else if (play_card_MessageObj.card_Arr.length===5) {
									// kind_32();
									// if(!kind_32()){ kind_105(); }
									kind_105();
									// if(!kind_105()){ kind_32(); }
									if(play_card_MessageObj.card_Type!==105){ console.log('in if?'); kind_32(); }
								}
								else{
									// kind_1Or5plus();
									alert('你走到了else那儿')
								...
								console.log(game_manager.Isturn);
							...
							switch(play_card_MessageObj.card_Arr.length){
								...
								case 3:
									if (play_card_MessageObj.card_Type === 30 || play_card_MessageObj.card_Type === null) {
										playChange(game_manager.nowPlayer)
									}
								break;
								...
								case 5:
									// console.log('看看这里有没有出现两次');
									if (play_card_MessageObj.card_Type === 105 || play_card_MessageObj.card_Type === null) {
										//符合出牌规则时
										//根据偏移样式反推牌的点数
									}
										playChange(game_manager.nowPlayer)
